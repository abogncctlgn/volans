...
* Time: 2020.08.24
* Description: 实现px4 quadrotor 使用yolo与kcf任意单一目标跟踪
***************************************************************************************************************************/
#include "tracking_kcf_quadrotor.h"
using namespace std;
using namespace Eigen;
PX4KcfTracking::PX4KcfTracking(const ros::NodeHandle& nh, const ros::NodeHandle& nh_private):
  nh_(nh),
  nh_private_(nh_private) {
  Initialize();
  cmdloop_timer_ = nh_.createTimer(ros::Duration(0.1), &PX4KcfTracking::CmdLoopCallback, this); //周期为0.1s
  //订阅目标相对飞机在图像像素点的位置
  ...
  position_sub_ = nh_private_.subscribe("/mavros/local_position/pose", 1, &PX4KcfTracking::Px4PosCallback,this,ros::TransportHints().tcpNoDelay());

  state_sub_ = nh_private_.subscribe("/mavros/state", 1, &PX4KcfTracking::Px4StateCallback,this,ros::TransportHints().tcpNoDelay());
  // 【服务】修改系统模式
  set_mode_client_ = nh_private_.serviceClient<mavros_msgs::SetMode>("/mavros/set_mode");

}

PX4KcfTracking::~PX4KcfTracking() {
  //Destructor
}

/**
* @name       S_SETPOINT_VEL PX4KcfTracking::TRACKINGPidProcess(Eigen::Vector3d &currentPos,Eigen::Vector3d &expectPos)

* @brief      pid控制程序,机体坐标系下控制无人机
*             
* @param[in]  &currentPos 当前飞机相对目标的位置
*             
* @param[in]  &expectPos 期望位置 expectPos[0]:相对目标前后方向距离；expectPos[1]:相对目标左右方向距离；expectPos[2]:相对目标上下方向距离
* @param[out] y,z的期望速度,以及yaw方向的期望速度。
*
* @param[out] 
**/
Eigen::Vector3d PX4KcfTracking::TrackingPidProcess(Eigen::Vector3d &currentPos,Eigen::Vector3d &expectPos)
{
...
void PX4KcfTracking::CmdLoopCallback(const ros::TimerEvent& event)
{
  TrackingStateUpdate();
}
...
/**
* @name       void PX4KcfTracking::TrackingStateUpdate()
* @brief      状态机更新函数
*             
* @param[in]  无
*             
...
* @param[out] 
**/
void PX4KcfTracking::TrackingStateUpdate()
{
...
/*接收来自飞控的当前飞机位置*/                  
void PX4KcfTracking::Px4PosCallback(const geometry_msgs::PoseStamped::ConstPtr &msg)
{
...
/*接收来自飞控的当前飞机状态*/
void PX4KcfTracking::Px4StateCallback(const mavros_msgs::State::ConstPtr& msg)
{
	px4_state_ = *msg;
}

/*初始化*/
void PX4KcfTracking::Initialize()
{
...
int main(int argc, char** argv) {
  ...
  PX4KcfTracking PX4KcfTracking(nh, nh_private);
